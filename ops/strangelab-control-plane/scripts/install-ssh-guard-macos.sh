...
PUBKEY_PATH="$1"
ALLOW_SRC="$2"
NODE_ID="${3:-$(scutil --get LocalHostName 2>/dev/null || hostname -s)}"
...
if [[ "$ALLOW_SRC" == "mac1" || "$ALLOW_SRC" == "mac2" ]]; then
...
if ! id -u strangelab >/dev/null 2>&1; then
  sudo sysadminctl -addUser strangelab -shell /bin/bash -password "-" >/dev/null 2>&1 || true
fi
# Forced-command restrictions live in authorized_keys; shell must be a valid login shell
# or ssh will close before running the forced command.
sudo dscl . -create /Users/strangelab UserShell /bin/bash >/dev/null 2>&1 || true

sudo mkdir -p /Users/strangelab/.ssh /opt/strangelab
sudo chmod 700 /Users/strangelab/.ssh
sudo chown -R strangelab:staff /Users/strangelab/.ssh
...
if [[ -f /etc/strangelab-ssh-guard.env ]]; then
  # shellcheck disable=SC1091
...
echo "$AUTH_LINE" | sudo tee /Users/strangelab/.ssh/authorized_keys >/dev/null
sudo chmod 600 /Users/strangelab/.ssh/authorized_keys
sudo chown strangelab:staff /Users/strangelab/.ssh/authorized_keys

sudo tee /etc/strangelab-ssh-guard.env >/dev/null <<ENV
VAULT_INGEST_URL=${VAULT_INGEST_URL}
NODE_ID=${NODE_ID}
ALLOWLIST=/opt/strangelab/allowlist.json
ENV

echo "ssh guard installed on $(scutil --get LocalHostName 2>/dev/null || hostname -s)"
