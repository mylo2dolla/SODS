...
function usage() {
  logInfo("usage: node tools/flash.mjs --board waveshare-esp32p4 [--version <ver>] [--port auto|/dev/tty...] [--erase] [--dry-run]");
}

function ensureP4Image(esptool, filePath, label) {
...
function ensureChipMatch(esptool, board, port) {
  ...
  if (!text.includes("esp32p4") && chipNeedle && !text.includes(chipNeedle)) {
...
try {
  ...
  ensureP4Image(esptool, boot, "bootloader");
  ensureP4Image(esptool, fw, "firmware");
  ...
  if (args.erase) {
    ...
    runEsptool(esptool, ["--chip", "esp32p4", "--port", port, "--baud", baud, "erase_flash"], { cwd: appRoot });
  }
