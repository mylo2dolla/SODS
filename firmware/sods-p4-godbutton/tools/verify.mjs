...
function usage() {
  logInfo("usage: node tools/verify.mjs --board waveshare-esp32p4 --version <ver>");
...
function verifyBoard(boardId, version) {
  ...
  const manifestPath = path.join(webToolsRoot, "manifest-p4.json");
  ...
  for (const binary of binaries) {
    ...
    if (name === "bootloader.bin" || name === "firmware.bin") {
      requireP4Image(esptool, abs, name);
    }
  }
  logInfo(`[verify] sods-p4-godbutton ${boardId} OK`);
}
