...
public struct ONVIFDiscoveryOutcome: Sendable {
    public let results: [OnvifDiscoveryResult]
    public let multicastAvailable: Bool

    public init(results: [OnvifDiscoveryResult], multicastAvailable: Bool) {
        self.results = results
        self.multicastAvailable = multicastAvailable
    }
}

public struct ONVIFDiscovery {
    public static func discover(timeout: TimeInterval, log: ScannerCoreLogger? = nil) async -> [OnvifDiscoveryResult] {
        await discoverWithOutcome(timeout: timeout, log: log).results
    }

    public static func discoverWithOutcome(timeout: TimeInterval, log: ScannerCoreLogger? = nil) async -> ONVIFDiscoveryOutcome {
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                coreLog(log, .info, "WS-Discovery started (timeout \(String(format: "%.1f", timeout))s)")
                let outcome = discoverSync(timeout: timeout, log: log)
                coreLog(log, .info, "WS-Discovery finished: found \(outcome.results.count) device(s)")
                continuation.resume(returning: outcome)
            }
        }
    }

    private static func discoverSync(timeout: TimeInterval, log: ScannerCoreLogger?) -> ONVIFDiscoveryOutcome {
        let socketFD = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
        guard socketFD >= 0 else { return ONVIFDiscoveryOutcome(results: [], multicastAvailable: false) }
        defer { close(socketFD) }

        var multicastAvailable = true
        ...
        if withUnsafePointer(to: &bindSockaddr, { ptr in
            ...
            return ONVIFDiscoveryOutcome(results: [], multicastAvailable: false)
        }
        ...
        while Date() < deadline {
            ...
            let xaddrs = extractTag("XAddrs", from: xml).map(parseXAddrs) ?? []
        ...
        for result in results {
            coreLog(log, .info, "WS-Discovery found device \(result.ip)")
        }
        return ONVIFDiscoveryOutcome(results: results, multicastAvailable: multicastAvailable)
    }
