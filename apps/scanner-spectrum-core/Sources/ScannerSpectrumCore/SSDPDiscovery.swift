...
public struct SSDPDiscoveryResult: Hashable, Sendable {
    ...
    public init(ip: String, st: String?, usn: String?, location: String?, server: String?, cacheControl: String?) {
        self.ip = ip
        self.st = st
        self.usn = usn
...
public struct SSDPDiscoveryOutcome: Sendable {
    public let results: [SSDPDiscoveryResult]
    public let multicastAvailable: Bool

    public init(results: [SSDPDiscoveryResult], multicastAvailable: Bool) {
...
public enum SSDPDiscovery {
    public static func discover(
        ...
        await discoverWithOutcome(timeout: timeout, log: log, onResult: onResult).results
    }

    public static func discoverWithOutcome(
        timeout: TimeInterval,
        log: ScannerCoreLogger? = nil,
        onResult: ((SSDPDiscoveryResult) -> Void)? = nil
    ) async -> SSDPDiscoveryOutcome {
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .utility).async {
                ...
                guard sock >= 0 else {
                    continuation.resume(returning: SSDPDiscoveryOutcome(results: [], multicastAvailable: false))
                    return
                }
                ...
                if sent <= 0 {
                    ...
                    coreLog(log, .warn, "SSDP send failed")
                }
                ...
                close(sock)
                continuation.resume(returning: SSDPDiscoveryOutcome(results: results, multicastAvailable: multicastAvailable))
            }
        }
    }
